...
services/pdf/          the Rust renderer (Cargo, not Yarn) - see §3 "PDF rendering"
...
Package manager is **Yarn 4** (`yarn.lock`, `nodeLinker: node-modules`; the release is pinned in `.yarn/releases` via `yarnPath`). TS `target` is `es2017`.

### Scripts

| Script | Command |
|---|---|
| `yarn dev` | `node server.js` (Next in dev mode behind the custom server) |
| `yarn build` | `next build` — type-checks and lints |
| `yarn start` | production custom server (needs a prior build) |
| `yarn test` | both suites: `test:api` then `test:web` |
| `yarn test:api` | `node --test "server/__tests__/**/*.test.js"` |
| `yarn test:web` | `node --test "src/__tests__/**/*.test.ts"` |
| `cargo test --manifest-path services/pdf/Cargo.toml` | the renderer's own tests (needs a Rust toolchain) |
| `yarn icons` | regenerate the PWA icons |
| `yarn themes` | expand `shared/themes.json` into `src/app/themes.css`, `src/lib/themes.generated.ts` and `shared/themes.generated.json` (the hex the PDF renderer reads) |
| `yarn variants` | expand `shared/variants/*.json` into `src/lib/variants.generated.ts` |
| `yarn fonts` | refresh `shared/google-fonts.json` from Google's public metadata |
| `yarn lint` | `next lint` |
...
* The theme travels as resolved hex, from `shared/themes.generated.json` - the
  reason `yarn themes` now emits JSON as well as CSS and TS. Neither CJS Node
...
(`yarn variants`). The generated step exists for the same reason the themes
have one: `src/lib/variants.ts` is covered by `yarn test:web`, which runs on
...
- **PWA**: `src/app/manifest.ts` plus `public/sw.js`, registered in production
  ...
  than an honest failure. Icons are generated by `yarn icons`.
...
`src/lib/themes.generated.ts`. **Never edit either by hand; run `yarn themes`.**
...
being an SSRF. `shared/google-fonts.json` (`yarn fonts`) backs the typeahead;
...
`yarn test:api` runs 97 tests on Node's built-in runner against
...
`yarn test:web` runs 23 tests over `src/`, also on node's runner: node 24
