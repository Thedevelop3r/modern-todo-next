...
const problems = [];
const blocks = [];
/** id -> { light, dark }, in hex, for the consumers that cannot read CSS. */
const palettes = {};

/**
 * The tokens the PDF renderer needs, as hex.
 *
 * The CSS above stores channels as "R G B" so Tailwind's alpha modifier works.
 * Neither the CommonJS API nor the Rust service can read CSS, and both need the
 * account's palette to make a PDF match the app - hence a third output.
 */
const PDF_TOKENS = ["bg", "surface", "fg", "fg-muted", "primary", "primary-fg", "border"];
...
for (const seed of seeds) {
  ...
  for (const mode of ["light", "dark"]) {
    ...
    palettes[seed.id] ??= {};
    palettes[seed.id][mode] = Object.fromEntries(
      PDF_TOKENS.map((token) => [
        // camelCase, because this is consumed as JSON rather than as CSS.
        token.replace(/-([a-z])/g, (_, letter) => letter.toUpperCase()),
        toHex(tokens[token]),
      ])
    );
  }
...
writeFileSync(
  join(root, "shared/themes.generated.json"),
  `${JSON.stringify(
    {
      _comment:
        "GENERATED by scripts/generate-themes.mjs - do not edit. Hex palettes for consumers that cannot read CSS (the CommonJS API and the Rust PDF service).",
      palettes,
    },
    null,
    2
...
console.log(
  `themes.css: ${seeds.length} themes (${Object.entries(byCollection)
    ...
    `themes.generated.json: ${Object.keys(palettes).length} palettes x light/dark.`
);
